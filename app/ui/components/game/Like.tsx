...
type LikeGameProps = { id: number; className?: string };

export function LikeGame({ id, className }: LikeGameProps) {
    ...
    useEffect(() => {
        setIsFav(favorites.includes(id.toString()));
    }, [favorites, id]);
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (isFav) {
        await removeFavoriteFromStore(id);
      } else {
        await addFavoriteToStore(id);
      }
    } catch (err) {
  ...
  return (
    <button type="button" onClick={toggleFavorite} className={className}>
