...
export function Game({ gameInfo }: { gameInfo: GameType }) {
  ...
  useEffect(() => {
    setIsFav(favorites.includes(gameInfo.id.toString()));
  }, [favorites, gameInfo.id]);

  const toggleFavorite = async () => {
    if (session?.user) {
      try {
        if (isFav) {
          await removeFavoriteFromStore(gameInfo.id);
        } else {
          await addFavoriteToStore(gameInfo.id);
        }
      } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-y-4 rounded-md overflow-hidden relative">
      <Link
        href={`/games/${gameInfo.slug}`}
        className="w-full aspect-video rounded-md overflow-hidden hover:*:scale-105"
      >
      ...
      <div className="flex flex-col gap-y-2">
        ...
        <h3 className="text-lg text-white/70 font-semibold hover:text-white transition-colors">
          <Link href={`/games/${gameInfo.slug}`}>{gameInfo.name}</Link>
        </h3>
      </div>
    </div>
  );
