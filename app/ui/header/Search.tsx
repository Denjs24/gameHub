...
export function Search () {
    ...
    return (
        <form className="flex items-center gap-x-2 relative z-1 flex-auto relative max-w-md">
            ...
            <input type="text" name='search' onFocus={() => setIsFocus(true)} onBlur={() => {
              setIsFocus(false)
              handlerChange('')
              }} defaultValue={searchParams.get('search')?.toString() || ''} onChange={e => handlerChange(e.target.value)} placeholder="Search"
